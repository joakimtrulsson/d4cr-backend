...
var permissions = {
  ...
  canCreateChapters: ({ session: session2 }) =>
    session2?.data.role?.canCreateChapters ?? false,
  canManageAllItems: ({ session: session2 }) =>
    session2?.data.role?.canManageAllItems ?? false,
...
var rules = {
  ...
  canManageItems: ({ session: session2 }) => {
    ...
    if (session2.data.role?.canManageAllItems) return true;
    return { contentOwner: { some: { id: { equals: session2.itemId } } } };
  },
  canReadUsers: ({ session: session2 }) => {
    if (!session2) return false;
    if (session2.data.role?.canSeeOtherUsers) return true;
    return { id: { equals: session2.itemId } };
  },
  canUpdateUsers: ({ session: session2 }) => {
    if (!session2) return false;
    if (session2.data.role?.canEditOtherUsers) return true;
    return { id: { equals: session2.itemId } };
  },
};
  ...
  ui: {
    ...
    itemView: {
      defaultFieldMode: ({ session: session2, item }) => {
        if (session2?.data.role?.canEditOtherUsers) return 'edit';
        if (session2?.itemId === item.id) return 'edit';
        return 'read';
      },
    },
  },
    ...
    password: (0, import_fields.password)({
      access: {
        ...
        update: ({ session: session2, item }) =>
          permissions.canManageUsers({ session: session2 }) ||
          session2?.itemId === item.id,
      },
...
var chapterSchema = (0, import_core3.list)({
  ...
  ui: {
    ...
    itemView: {
      defaultFieldMode: ({ session: session2, item }) => {
        if (session2?.data.role?.canManageAllItems) return 'edit';
        if (session2.data.chapters[0].id === item.id) return 'edit';
        return 'read';
      },
    },
    labelField: 'title',
